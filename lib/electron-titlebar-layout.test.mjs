...
test("会话行的操作菜单也 portal 到 body（否则被行内 overflow 裁掉）", () => {
  // 真机反馈：菜单只有一小部分可见、文字看不见 —— 根因是菜单渲染在带 overflow-hidden 的行内，
  // 祖先一旦成为 fixed 的包含块（Safari 尤其敏感）就会被裁掉大半。
  const tree = readFileSync(
    new URL("../components/session-sidebar/SessionTree.tsx", import.meta.url),
    "utf8",
  );
  assert.match(tree, /createPortal\(\s*<div\s+ref=\{menuRef\}/);
  assert.match(tree, /document\.body,\s*\)\}/);
  assert.match(tree, /useDismissOnOutsideClick\(\[menuRef, moreButtonRef\]/);
  assert.match(tree, /menuStyle\.top/);
});
