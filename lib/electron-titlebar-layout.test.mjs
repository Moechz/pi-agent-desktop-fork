...
test("language row cannot push the select outside the menu", () => {
  // 真机截图：中文切英文后 “Language” 变长，原生 select 固有宽度又大，
  // 两者叠加把选择框顶出 w-52 的弹窗。修法是让 flex 子项能收缩（min-w-0）并给 select 上限。
  assert.match(appShellSource, /<label className="flex w-full min-w-0 items-center gap-2/);
  assert.match(appShellSource, /<span className="min-w-0 flex-1 truncate">\{t\("language\.label"\)\}/);
  assert.match(appShellSource, /className="min-w-0 max-w-\[58%\] shrink[^"]*"[^]*<option value="system"/);
  // 弹窗自身再上一层兜底：任何语言下文案变长都不会溢出到弹窗外
  assert.match(
    appShellSource,
    /id="workbench-menu"[^]*className="[^"]*w-52 overflow-hidden[^"]*"/,
  );
});
